...
export default function ExcelTable({
    ...
    const rows = arr.map(row => {
      return columns.map(col => {
        ...
        if (col.cell && typeof col.cell === 'function') {
          // Attempt plain string representation if cell customizer is present
  ...
  const renderStatusBadge = (val) => {
    ...
    if (cleanVal === 'active' || cleanVal === 'completed' || cleanVal === 'compliant') {
      badgeClass = 'badge-active';
    } else if (cleanVal === 'pending' || cleanVal === 'pending_review') {
      badgeClass = 'badge-pending';
    } else if (cleanVal === 'in_progress') {
      badgeClass = 'badge-in_progress';
    } else if (cleanVal === 'non-compliant' || cleanVal === 'non_compliant' || cleanVal === 'suspended') {
        ...
        <table className="excel-table">
          <thead>
            <tr>
              ...
              {actions.length > 0 && <th style={{ width: '100px', minWidth: '100px', textAlign: 'center' }}>Actions</th>}
            </tr>
          </thead>
          <tbody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row, rIdx) => (
                ...
                >
                  ...
                  {actions.length > 0 && (
                    <td className="actions-td" style={{ textAlign: 'center', width: '100px' }} onClick={(e) => e.stopPropagation()}>
                      <div className="actions-cell-wrapper">
                        ...
                        {openActionRowId === (row.id || rIdx) && (
                          <div className="actions-dropdown-menu" style={dropdownStyle}>
