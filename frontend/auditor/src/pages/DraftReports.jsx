...
const DraftReports = () => {
  ...
  // ── Real .docx report generated by the backend ──
  const [isGeneratingReport, setIsGeneratingReport] = useState(false);

  const handleDownloadWordReport = async () => {
    if (!activeAuditData.frameworkId) {
      alert('No audit is selected, so there is nothing to generate a report for.');
      return;
    }
    if (isGeneratingReport) return;
    setIsGeneratingReport(true);
    try {
      await downloadAuditReport(activeAuditData.frameworkId);
    } catch (err) {
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <div className="page-header-actions" style={{ display: 'flex', gap: '10px' }}>
          <button
            className="btn btn-primary"
            style={{ display: 'flex', alignItems: 'center', gap: '6px', opacity: isGeneratingReport ? 0.7 : 1 }}
            onClick={handleDownloadWordReport}
            disabled={isGeneratingReport || !activeAuditData.frameworkId}
          >
            <FileText size={16} /> {isGeneratingReport ? 'Generating...' : 'Generate Word Report'}
          </button>
          <button className="btn btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '6px' }} onClick={handleGenerateDraft}>
